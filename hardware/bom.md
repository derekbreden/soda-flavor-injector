...
- **Rear-panel C14 inlet bezel recess (enclosure design directive).** When the outer enclosure rear panel is printed (§7 "Outermost enclosure" mass allocation), recess the C14 inlet **3–5 mm** into the panel face with a printed shroud around the inlet perimeter. The cord housing nests into the recess on insertion and ends flush with the rear panel surface, visually masking the IEC-mandated gap between cord and inlet bezel. Pure fit/feel improvement; no electrical or mechanical change to the connector itself. Background: the C13/C14 cross-test pair (uxcell inlet B07PXSLBF4 + Tripp Lite cord B0000511C0, ordered Apr 24, 2026, arrived Apr 25) confirmed all four parts in the MXR×Monoprice / MXR×TrippLite / uxcell×Monoprice / uxcell×TrippLite matrix mate to spec — the visible gap between cord housing and inlet face is by design under IEC 60320, which specifies only the male-blade insertion region, not face-to-face mating distance. For a Founder Edition appliance, the printed shroud is the cheapest path to a "fully seated" appearance. Locking C13 cords (Tripp Lite P-Lock series) were considered and rejected: the user concern is fit/feel feedback, not mechanical retention, and friction-only retention is sufficient for an under-counter install. Hardwired-cord option (KitchenAid/Vitamix pattern) held under consideration as an unrelated alternative direction.
