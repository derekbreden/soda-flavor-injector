...
Body: two 7.754" × 6.000" half-blanks in 0.048" 304 SS, formed into D-halves on the 12-ton hydraulic press using a printed PA6-CF die set, then butt-welded together along both flat sides to close the racetrack. The slip roll (VEVOR 24") was the original forming plan but was abandoned because captive rollers can't release a closed-loop part, and even the D-half variant would rely on operator-eyeballed bend angles with 3–8% springback per part. Press-forming with a die pair gives identical repeatable parts with over-bend compensation built in, at the cost of two printed dies. Each die pair forms one D-half per stroke; qty 2 of each die lets both halves be formed in parallel. D-half cross-section: two 0.800" tangent walls flanking a 1.935"-radius semicircle (inside), giving body ID R=1.935", flat=1.600" (5.470" × 3.870"). Outside: R=1.983", flat=1.600" (5.566" × 3.966"). Hoop stress at 70 PSI: 4,058 PSI — 4.9× safety factor vs 20,000 PSI allowable for 304 SS. The two longitudinal butt welds carry hoop stress as continuous membrane (full-penetration butt welds on passivated 304 SS exceed parent-material strength). Blank DXF in `cut-parts/carbonator-body-sheet/generate_dxf.py`; press die specs and CadQuery script in `printed-parts/carbonator-body-press-dies/`.
...
The tank interior is passivated with citric acid (one-time, 30-60 min soak of ~4% food-grade citric acid solution in a disposable plastic tub, then thorough water rinse) after welding and before first service. Passivation restores the chromium oxide layer at the weld zones, which is what makes 304 SS resistant to pitting corrosion from carbonic acid (carbonated water, pH ~3.5-4). This is the same treatment commercial brewery bright tanks and commercial carbonators receive. A food-safe epoxy coating was considered but rejected: commercial carbonators (Lillium, Brio, SodaStream commercial) all run bare passivated 304/316 SS for decades, and epoxy coatings can fail at pinholes and concentrate corrosion where a uniform oxide layer would spread it harmlessly. The vessel is hydro-tested to 150 PSI (1.5x working pressure) for 30 minutes before service as a verification step.
