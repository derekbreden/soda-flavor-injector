...
Flavor-reservoir Plan B is a pair of custom printed hard reservoirs, one per flavor, conforming directly to the cold-core envelope instead of forcing off-the-shelf bottle geometry into the under-sink package. The reservoirs are vented, not service-pressure vessels: 1L usable volume target, low outlet sump, high filtered vent, fill/dispense/clean paths through the same valve manifold. For Plan B the reservoirs remain separate printed parts that fit inside the foam-bag shell; an integrated reservoir/shell print is a later Plan C idea. Today's foam-bag-shell pockets are just under 1L internal volume, so the expected package change is a small X-axis pocket enlargement without disturbing the Y/Z pressure-vessel and plumbing envelope. The test plan and candidate food-contact PET-family filaments live in `printed-parts/plan-b/reservoir/README.md`. Plan B only replaces the bladder/cradle physical package; the user experience and manifold behavior stay the same.
