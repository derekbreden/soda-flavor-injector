...
Body: two 7.754" × 6.000" half-blanks in 0.048" 304 SS. The VEVOR 24" slip roll has captive rollers (no top-roller release, no axial opening), so pieces can only exit out the back, in the rolling direction. A full-perimeter racetrack blank would close on itself mid-roll — the leading edge curls around and jams against the trailing edge right where new material is still feeding through the nip. Splitting the racetrack into two symmetric D-halves keeps the leading and trailing edges separated by the semicircle diameter (~3.9") at all times, so each piece exits the back cleanly and drops free. Each half rolls into a 180° arc with a short flat tangent on each end (flat 0.800" → semicircle 6.154" → flat 0.800"), and the two halves are butt-welded together along both flat sides (two longitudinal seams). Rolled ID: R=1.935", flat=1.600" (5.470" × 3.870"). OD: R=1.983", flat=1.600" (5.566" × 3.966"). Hoop stress at 70 PSI: 4,058 PSI — 4.9× safety factor vs 20,000 PSI allowable for 304 SS. The weld seams carry hoop stress as continuous membrane (full-penetration butt welds on passivated 304 SS exceed parent-material strength). Blank and rolling zone dimensions are in `cut-parts/carbonator-body-sheet/generate_dxf.py`.
