...
| Material | PETG |
| Envelope (W × H × D) | 137.2 mm × 68.6 mm × 95.0 mm (plate 137.2×68.6×5; struts extend 90 mm from user-facing face) |
| Print orientation | Fitting-facing face DOWN on build plate (XZ plane on build plate). Struts extend upward (+Y print direction). Stepped bore counterbores open upward away from build plate. |
| Piece count | 1 |
| Fasteners | None |
| Sub-assemblies | None |
...
| Y | Depth, fitting-facing to user-facing; struts extend beyond user-facing face | 0 → 95.0 mm |
| Z | Height, bottom to top | 0 → 68.6 mm |

Origin: bottom-left corner of the plate at the fitting-facing face (X=0, Y=0, Z=0).
...
**v4** moves the 4 struts from the fitting-facing face to the user-facing face (Y=5.0 → Y=95.0), and removes the 2 guide pins entirely. The guide pin bores, return springs on pins, and related interfaces are all moved to other parts in the assembly — they are not features of this plate.
...
Force path: user finger pads contact the plate user-facing face (Y=5) → force transmits through the 5.0 mm plate body in tension along Y → annular lip faces at the Zone 2→Zone 3 step transition (Y=1.6 mm) contact the PP0408W collet annular end faces → collets translate inward (toward the user) → collet spring compression → collet gripper teeth release the dock tube stubs.
...
**Translational constraint (over-travel):** The Zone 1 outer bore floor acts as a hard stop. When the plate has traveled 1.33 mm toward the user, the Zone 1 bore floor (at Y=3.6 mm = 1.4 mm depth from user-facing face) contacts the PP0408W body-end shoulder (15.10 mm OD annular face). The fitting body becomes the over-travel stop.
...
The user approaches the installed cartridge with one hand, palm up. The palm presses against the cartridge body front face rim. The fingers curl into the 10 mm deep pocket and contact the plate user-facing face (Y=5). The user squeezes: fingers pull, palm pushes. Resistance builds as the four collet springs compress. At approximately 1.33 mm of plate travel, all four collets simultaneously clear their internal stops — the gripping teeth release the dock tube stubs — and the user feels and hears four simultaneous clicks. The resistance drops sharply. The user's hand continues pulling the cartridge forward out of the bay on its protruding tracks.
...
### Feature 3 — Pull Edge Radius (Perimeter Fitting-Face Edge)
...
| Radius | 3.0 mm |
| Justification | Tactile comfort: the radius is on the fitting-side edge the finger pads bear against during the pull stroke. This surface must not feel sharp. requirements.md §6 dimensional accuracy elephant's foot requirement is fully satisfied by this 3.0 mm radius. |
...
| Base Y | Y = 5.0 mm (user-facing face of plate) |
...
| Base Y | Y = 5.0 mm (user-facing face) |
...
|---|---|
| Elephant's foot chamfer (0.3 mm × 45°) | The 3.0 mm pull edge radius (Feature 3) applied to all four fitting-face perimeter edges fully satisfies requirements.md §6 elephant's foot requirement. No separate chamfer needed. |
...
### Interface 5: Plate fitting-facing face (Y=0) ↔ Cartridge body pocket floor (at rest)
...
2. **Insert plate into cartridge body pocket:** Lower the plate into the cartridge body pocket with the user-facing face (Y=5) facing outward and the struts extending toward the lever.
