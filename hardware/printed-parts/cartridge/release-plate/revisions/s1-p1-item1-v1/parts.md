...
| Print orientation | Fitting-facing face DOWN on build plate (XZ plane on build plate). Guide pins extend upward (+Y print direction). All stepped bore features open downward toward build plate. |
| Piece count | 1 |
| Fasteners | None |
| Sub-assemblies | None |
...
### Feature 3 — Pull Edge Radius (Perimeter User-Facing Face Edge)

| Field | Value |
|---|---|
| Operation | Remove (blend, CadQuery fillet) |
| Shape | Convex quarter-circle fillet, R = 3.0 mm |
| Affected edges | All four perimeter edges at the user-facing face plane (Y=5): left edge (X=0, Y=5, Z: 0→65.0); right edge (X=80.0, Y=5, Z: 0→65.0); bottom edge (Z=0, Y=5, X: 0→80.0); top edge (Z=65.0, Y=5, X: 0→80.0). The effective length of each edge is reduced at the corners by the interaction with the 2.0 mm corner radius (Feature 2). Apply Feature 2 first; Feature 3 acts on the remaining edge lengths. |
| Radius | 3.0 mm |
| Radius center location | Inset 3.0 mm from both the user-facing face plane (Y=2.0) and each perimeter wall plane. Example for left edge: arc center at (X=3.0, Y=2.0) in the XY cross-section. |
...
Coordinate system: X = width (0→80 mm), Y = depth fitting-facing-to-user-facing (0=fitting-facing face, 5=user-facing face; +Y = toward user), Z = height (0→65 mm). Cartridge assembly: +Y = rearward (away from user), −Y = toward user.
...
|---|---|---|---|---|
| Plate body + Guide Pins 1 & 2 | No (pins are integral; they translate with the plate as a unit) | Yes (PETG) | Yes (fitting-facing-face-down orientation, pins point up — no overhang) | COMBINED correctly. Integral pins eliminate press-fit assembly, tolerance stack, and potential pin-loosening under cycling. |
...
- The fitting-facing face (build plate face) is the first-layer face — maximum surface quality and dimensional accuracy.
