...
| Material | PETG |
| Envelope (W × H × D) | 80.0 mm × 65.0 mm × 95.0 mm (plate 80×65×5; struts extend 90 mm from fitting-facing face) |
| Print orientation | Fitting-facing face DOWN on build plate (XZ plane on build plate). Guide pins and struts extend upward (+Y print direction). All stepped bore features open downward toward build plate. |
| Piece count | 1 |
| Fasteners | None |
| Sub-assemblies | None |
...
| Y | Depth, fitting-facing to user-facing | 0 → 5.0 mm |
| Z | Height, bottom to top | 0 → 65.0 mm |

Origin: bottom-left fitting-facing corner of the plate (the corner at X=0, Y=0, Z=0).
Y=0 = fitting-facing face (tube exit side, sits on build plate in print orientation).
Y=5 = user-facing face (bore-entry face, pull surface).
...
Force path: user finger pads contact the plate user-facing face (Y=5) → force transmits through the 5.0 mm plate body in tension along Y → annular lip faces at the Zone 2→Zone 3 step transition (Y=1.6 mm) contact the PP0408W collet annular end faces → collets translate inward (in the +Y direction relative to the cartridge, away from the user) → collet spring compression → collet gripper teeth release the dock tube stubs.

### What constrains the plate

**Rotational constraint:** Two integral guide pins (Pin 1 at X=5.0, Z=60.0; Pin 2 at X=75.0, Z=5.0) extend 30.0 mm from the plate user-facing face (Y=5.0 to Y=35.0 in part frame) and slide in matched cylindrical bores in the cartridge body. The diagonal placement of the two pins (89.0 mm center-to-center) prevents any rotation of the plate about the Y axis (or any other axis) during translation. The 0.5 mm designed diametric clearance between pin OD (5.0 mm) and bore ID (5.5 mm) limits maximum tilt to approximately 0.31°.

**Translational constraint (over-travel):** The Zone 1 outer bore floor acts as a hard stop. When the plate has traveled 1.33 mm toward the user, the Zone 1 bore floor (at Y=3.6 mm from fitting-facing face = Y=1.4 mm depth from user-facing face) contacts the PP0408W body-end shoulder (15.10 mm OD annular face). The fitting body becomes the over-travel stop. The designed plate stroke of 3.0 mm provides 0.07 mm over-travel margin beyond the collet release depth (1.33 mm) — the fitting body hard stop prevents over-travel damage.
...
| Justification (elephant's foot) | requirements.md §6 dimensional accuracy: "If the bottom face is a mating surface, add a 0.3 mm × 45° chamfer to the bottom edge." The fitting-facing face (Y=0) is the build plate face and a mating surface (mates with cartridge body pocket floor). The 3.0 mm radius is far larger than the 0.3 mm chamfer minimum and fully satisfies this requirement on all four build-plate-contact edges. No separate elephant's foot chamfer is needed. |
| Note on print orientation | In print orientation (fitting-facing face down), this radius is at the very bottom of the part — the perimeter of the first printed layers. It prints as a convex curve on the part exterior, fully supported by the surrounding build plate contact. No overhang concern. |
...
| Zone 1 — Outer bore | Ø 15.60 mm; depth from user-facing face (Y=5.0): 1.4 mm; Y range: 5.0 → 3.6 mm |
| Zone 2 — Inner lip bore | Ø 10.07 mm; depth: 2.0 mm beyond Zone 1; Y range: 3.6 → 1.6 mm |
| Zone 3 — Tube through-hole | Ø 6.50 mm; depth: 1.6 mm through remaining plate; Y range: 1.6 → 0.0 mm (exits fitting-facing face) |
...
| Field | Value |
|---|---|
| Operation | Add (integral cylindrical boss protruding from user-facing face) |
| Shape | Right circular cylinder |
| Diameter | 5.0 mm (OD) |
| Center position (X, Z) | X = 5.0 mm, Z = 60.0 mm |
| Base Y | Y = 5.0 mm (at plate user-facing face — pin is flush with user-facing face, no inset) |
| Tip Y | Y = 35.0 mm |
| Length from user-facing face | 30.0 mm |
...
| Field | Value |
|---|---|
| Operation | Add (rectangular boss protruding from fitting-facing face) |
| Shape | Rectangular prism |
| Cross-section (X × Z) | 6.0 mm × 6.0 mm |
| Length (Y) | 90.0 mm |
| Center position (X, Z) | X = 9.0 mm, Z = 60.0 mm |
| Box extents | X: 6.0→12.0, Y: −90.0→0.0, Z: 57.0→63.0 |
| Base Y | Y = 0.0 mm (fitting-facing face of plate — strut base is flush with fitting-facing face) |
...
| Nearest plate edge | Bottom plate edge Z=0.0; strut bottom at Z=2.0 → 2.0 mm from bottom edge. The strut projects in −Y from the fitting-facing face and does not interact with the plate perimeter walls. |
...
|---|---|
| Elephant's foot chamfer (0.3 mm × 45°) | The 3.0 mm pull edge radius (Feature 3) applied to all four fitting-facing-face perimeter edges fully satisfies requirements.md §6 elephant's foot requirement. No separate chamfer needed. |
| Spring pockets | Springs sit in pockets in the cartridge body rear wall, not in the plate. Plate user-facing face (Y=5.0 mm) is the spring contact surface — flat, no feature required. |
| Embossed texture on pull surface | The user-facing face is smooth (satin PETG as-printed). Smooth is correct for the pull surface — finger pads must slide slightly during the curl motion. The cartridge body palm surface carries the matte embossed texture. |
...
| Zone 1 depth from user-facing face | 1.4 mm | Collet travel to release ~1.33 mm | 0.07 mm over-travel margin before hard stop | geometry-description.md; synthesis.md §3 |
...
| Pin length | 30.0 mm from user-facing face | Bore depth ≥ 30.0 mm | 0 mm at rest; pin fully in bore; 3 mm of pin enters bore during stroke → 27 mm retained at full stroke | spatial-resolution.md §3.5 |
...
### Interface 5: Plate user-facing face (Y=5.0) ↔ Return springs

| Dimension | Plate side | Cartridge body side | Notes |
|---|---|---|---|
| Spring contact surface | Flat user-facing face, Y=5.0 mm, no feature | Spring end coil; spring OD 7–8 mm, coil wraps guide pin | No plate feature needed; spring guided by cartridge body pocket |
| Pin 1 spring contact zone | User-facing face annulus centered (5.0, 60.0), r_in=2.5 mm, r_out=4.0 mm | 8 mm diameter pocket concentric with pin 1 bore | Spring wraps pin |
| Pin 2 spring contact zone | User-facing face annulus centered (75.0, 5.0), r_in=2.5 mm, r_out=4.0 mm | 8 mm diameter pocket concentric with pin 2 bore | Spring wraps pin |

### Interface 6: Plate fitting-facing face (Y=0) ↔ Cartridge body pocket floor (at rest)

| Dimension | Plate side | Cartridge body side | Notes |
|---|---|---|---|
| Fitting-facing face position (at rest) | Y=0 mm in part frame | Pocket floor at 10 mm depth from cartridge body front face | 10 mm inset for finger grip ergonomics |
...
2. **Insert guide pins into cartridge body bores:** With the cartridge body front pocket facing upward, lower the release plate user-facing face down toward the cartridge body interior. Align the two guide pins (user-facing face protrusions) with the two 5.5 mm bores in the cartridge body rear-wall structure. Lower the plate until the pins are fully seated in the bores (plate fitting-facing face approaches but does not contact the spring end coils at this stage — the springs are seated in their pockets first, see step 3).
...
4. **Insert plate over springs:** Lower the plate into the cartridge body pocket with the guide pins aligned to the bores. The plate user-facing face compresses the springs slightly as the pins enter the bores. The plate fitting-facing face should be approximately flush with or slightly below the cartridge body front face — it will settle to 10 mm inset when the cartridge assembly is complete and the plate rests against the collet end faces via the spring.
...
0. **User sees and touches:** Described — clean rectangular inset on cartridge front face, 10 mm deep, plate user-facing face is the floor of the inset. Parting line gap 0.6–1.0 mm described.

1. **What moves:** Release plate translates 0–3.0 mm in −Y. Cartridge body and fittings are stationary. Named explicitly.

2. **What converts the motion:** Direct force transmission, no mechanical advantage. Force path named: user-facing face → plate body in tension → annular lip at Y=1.6 mm → collet end faces → collet springs.
...
4. **Return force:** Two compression springs (8 mm OD pocket, 10 mm deep, ~1 N each), bearing on flat user-facing face Y=5.0 mm. Feature and dimensions named.

5. **User interaction:** Palm-up posture described. Finger pad contact with 3.0 mm radius pull edge (Feature 3) described with dimensional reference. Four simultaneous clicks at 1.33 mm travel described as tactile endpoint — grounded by: Zone 1 bore floor hard stop at Y=3.6 mm (depth from user-facing face 1.4 mm) contacting the 15.10 mm body-end shoulder. Force drop described qualitatively (30–50% drop per design-patterns.md reference).
...
[User hand: palm-up, fingers curl into 10mm inset pocket]
        ...
        v
[Plate user-facing face Y=5 — pull surface, 80×65mm flat PETG face]
        |
        ...
        | Returned to rest by: 2× compression springs bearing on user-facing face (Y=5.0mm)
        v
...
Coordinate system: X = width (0→80 mm), Y = depth fitting-facing to user-facing (0=fitting-facing face, 5=user-facing face), Z = height (0→65 mm). Cartridge assembly: +Y = rearward (away from user), −Y = toward user.
...
| "Pins extend from user-facing face rearward" | +Y from user-facing face | +Y | PASS | Pins go from Y=5.0 (user-facing face) to Y=35.0 in part frame; rearward = +Y in part frame |
| "Return springs push plate back to rest" | +Y (rearward) → springs push plate in −Y? | −Y net | PASS | Springs are in cartridge body rear-wall pockets, bear against plate user-facing face; spring compression pushes plate toward fitting face (+−Y) = toward user... CORRECTION: springs are compressed when plate moves toward user (−Y). At rest, springs are at natural length, bearing against plate user-facing face Y=5.0. When plate moves −Y (toward user), spring compresses. On release, spring expands, pushing plate in +Y direction (toward user) → plate returns to rest. Spring pushes in +Y = toward user. Plate rest position is maintained by collets (spring-loaded outward) stopping plate from going further +Y. Claim "springs return plate to rest" = correct; spring force direction is +Y (toward user face), which is the direction from actuated position back to rest. PASS. |
...
| "Plate user-facing face sits 10 mm behind cartridge front face" | +Y offset from cartridge front face plane | +Y in cartridge frame | PASS | At rest, plate user-facing face (part Y=5) is 10 mm in the +Y direction from the cartridge body front face. PASS. |
| "Zone 1 bore opens from user-facing face" | Opens at Y=5.0, deepens toward Y=3.6 mm | −Y from user-facing face | PASS | Zone 1: Y range 5.0→3.6 mm; bore enters at Y=5.0 (user-facing face) and extends 1.4 mm in −Y. PASS. |
| "Zone 3 exits at fitting-facing face" | Exits at Y=0 | −Y through plate | PASS | Zone 3: Y range 1.6→0.0 mm. Exits at Y=0 = fitting-facing face. PASS. |
| "Pins extend 30 mm from user-facing face" | From Y=5.0 to Y=35.0 in part frame | +Y in part frame (upward in print) | PASS | spatial-resolution.md §3.5 confirmed. PASS. |
...
| Plate body + Pull surface | No (pull surface is the plate user-facing face; it is not a separate cosmetic layer) | Yes | Yes | COMBINED correctly. A cosmetic face layer would run a joint through the primary load path (finger pull → plate body). |
...
**Orientation: Fitting-facing face DOWN (Y=0 face on build plate). XZ plane on build plate.**

Guide pins extend upward in +Y (away from build plate). All stepped bore features drill upward from the fitting-facing face.
...
- The fitting-facing face is the first-layer face — maximum surface quality and dimensional accuracy.
...
|---|---|---|---|---|
| Fitting-facing face (Y=0) | Horizontal face on build plate | 90° from horizontal (vertical face, but it's the BUILD PLATE contact — no overhang) | OK | Build plate face |
| User-facing face (Y=5.0 mm) | Top horizontal face of plate body | 90° from horizontal (horizontal top face) | OK | Top face, no overhang |
...
**No unresolved overhangs. All surfaces are printable without support in fitting-facing-face-down orientation.**
...
| Zone 3 through-hole top exit (at user-facing face, Z=5.0 mm in print) | The Zone 3 bore exits at Z=5.0 mm (the user-facing face, top of print). This is an open hole in the top face, not a bridge. | Not a bridge — open hole | N/A | PASS |
...
| Struts TL, TR, BL, BR (6×6 mm × 90 mm, at (9,60), (71,60), (9,5), (71,5)) | Physical necessity — assembly | vision.md §3 Season 1 Phase 2: "The struts connect to the release plate." Four struts required (vs 2) per vision.md §3: "especially if we use 4 struts." Positions derived from bore column X values (9.0 and 71.0) and Z values that clear all bore outer circles by 1.7 mm minimum. Struts extend from fitting-facing face (Y=0) in the −Y direction, on the opposite side from the guide pins (+Y), so no Y-space conflict. Plain ends per deliverable: "Plain rectangular struts — no joinery on the ends." |
| Flat user-facing face (no spring pockets, no bosses) | Physical necessity — assembly | Springs sit in cartridge body pockets; flat user-facing face is the spring contact surface. Simplicity eliminates features that would complicate print orientation or add unnecessary geometry. |
...
3. `fillet(3.0)` on the four fitting-facing-face perimeter edges (edges at Y=0 face) — pull edge radius
...
6. Four identical stepped bore cuts at centers (9.0, 47.5), (9.0, 17.5), (71.0, 47.5), (71.0, 17.5) [X, Z]:
   - Each bore uses a revolved profile cut from the user-facing face (Y=5.0), cutting toward the fitting-facing face:
...
**No supports required. Print fitting-facing face (Y=0) down.**
