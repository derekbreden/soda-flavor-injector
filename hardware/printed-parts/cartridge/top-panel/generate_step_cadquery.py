"""
Top Panel — CadQuery STEP Generation Script

Specification source: hardware/printed-parts/cartridge/top-panel/planning/parts.md

The top panel is a flat panel with no features. It spans the interior ceiling
of the pump cartridge, captured in the top panel channels of the left and
right walls (Z=73.6..77.0mm in wall coordinates, 3.4mm channel width).
It is the last piece inserted during assembly, sliding in from above (-Z).
...
# Script is at: hardware/printed-parts/cartridge/top-panel/generate_step_cadquery.py
...
FEATURE_TABLE = """
TOP PANEL — FEATURE PLANNING TABLE (Rubric 1)
=============================================
...
Assembly context:
  Top panel channel on left/right walls: Z=73.6..77.0mm in wall coordinates.
  Panel slides in from above (-Z direction), last piece in during assembly.
"""

print("=" * 70)
print("TOP PANEL — CadQuery STEP Generation")
print("=" * 70)
print(FEATURE_TABLE)
...
print(f"  [+] Feature 1: Panel body ({PANEL_W} x {PANEL_D} x {PANEL_T} mm, X x Y x Z)")
...
out_path = Path(__file__).resolve().parent / "top-panel-cadquery.step"
