...
# Derived
INTERIOR_X = WALL_T
RAIL_TIP_X  = WALL_T + RAIL_H
PASS_THRU_GAP = RAIL_W + CHANNEL_W  # total allowing pass through
...
def add_rail(y0, z0, rail_dy, rail_dz):
    return (
        cq.Workplane("XY")
...
BACK_PANEL_RAIL_INT_Y0 = 5.4  # RAIL_W + CHANNEL_W

rail_back_panel_ext = add_rail(
...
# Top panel / plate-top rails (shared rail)
...
# Y position, gapped at bottom and top for horizontal rails.
...
for y_center in [PUMP_TRAY_Y_CENTER, COUPLER_TRAY_Y_CENTER]:
    ...
    for cut_z0, cut_dz in [
        (BOTTOM_PANEL_RAIL_INT_Z0, RAIL_W),
        (TOP_PANEL_RAIL_INT_Z0, RAIL_W),
        (TOP_PANEL_RAIL_EXT_Z0, RAIL_W),
    ]:
        cutout = (
            cq.Workplane("XY")
