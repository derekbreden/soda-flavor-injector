...
- Springback: 304 SS at R/t ~= 40 typically springs back 3–8% on tight wraps; on this low R/t the semicircle portion will hold tight but the tangent walls may open slightly. The script exposes a `SPRINGBACK_COMP` constant (default `0.0`) that shrinks both die radii by the same fractional amount to over-form the part. Measure the first formed D-half against the end-cap slip fit (R = 1.930" tube ID target). If the semicircle is loose or the walls splay open, bump `SPRINGBACK_COMP` to 0.03–0.05, regenerate the STEPs, and reprint. The punch-to-cavity gap stays equal to sheet thickness at every point on the D regardless of the compensation value.
