...
The caller provides snap_base_in_wall (where the snap starts within the wall)
...
def apply_ramp_out_first(solid, inner_face, sign, plane, extrude_start,
                         zone_width, snap_base_in_wall, wall_height,
    ...
    snap_base_in_wall: coordinate where the snap features begin.
    wall_height:       total wall height available from snap_base_in_wall.
    ...
    wall_top = snap_base_in_wall + hd * wall_height
    ...
    growth = [
        _pt(oi,                          snap_base_in_wall + hd * OUTER_GROWTH_START, sw),
        _pt(outer + sign * OUTER_GROWTH, snap_base_in_wall + hd * f, sw),
        _pt(outer + sign * OUTER_GROWTH, snap_base_in_wall + hd * tip_h, sw),
        _pt(oi,                          snap_base_in_wall + hd * tip_h, sw),
    ]
    solid = solid.union(
    ...
    extension = [
        _pt(ic, wall_top, sw),
        _pt(ic, snap_base_in_wall + hd * tip_h, sw),
        _pt(oi, snap_base_in_wall + hd * tip_h, sw),
        _pt(oi, wall_top, sw),
    ]
    solid = solid.union(
    ...
    channel = [
        _pt(ic,         snap_base_in_wall + hd * f, sw),
        _pt(bump_face,  snap_base_in_wall + hd * f, sw),
        _pt(notch_face, snap_base_in_wall + hd * (f + r), sw),
        _pt(notch_face, snap_base_in_wall + hd * (f + r + e), sw),
        _pt(bump_face,  snap_base_in_wall + hd * (f + 2 * r + e), sw),
        _pt(bump_face,  snap_base_in_wall + hd * (f + 2 * r + 2 * e), sw),
        _pt(notch_face, snap_base_in_wall + hd * tip_h, sw),
        _pt(ic,         snap_base_in_wall + hd * tip_h, sw),
    ]
    solid = solid.cut(
...
def apply_ramp_in_first(solid, inner_face, sign, plane, extrude_start,
    ...
    if outer_growth > 0:
        ...
        growth = [
            _pt(oi,                            snap_base_in_wall + hd * s, sw),
            _pt(outer + sign * outer_growth,   snap_base_in_wall + hd * s, sw),
            _pt(outer + sign * outer_growth,   snap_base_in_wall + hd * tip_h, sw),
            _pt(oi,                            snap_base_in_wall + hd * tip_h, sw),
        ]
        solid = solid.union(
    ...
    extension = [
        ...
        _pt(ic,         snap_base_in_wall + hd * tip_h, sw),
        _pt(notch_face, snap_base_in_wall + hd * tip_h, sw),
        _pt(bump_face,  snap_base_in_wall + hd * (s + 2 * r + 2 * e), sw),
    ...
    notch_cut = [
        _pt(oc,         snap_base_in_wall + hd * s, sw),
        _pt(notch_face, snap_base_in_wall + hd * (s + r), sw),
        _pt(notch_face, snap_base_in_wall + hd * (s + r + e), sw),
        _pt(bump_face,  snap_base_in_wall + hd * (s + 2 * r + e), sw),
        _pt(bump_face,  snap_base_in_wall + hd * (s + 2 * r + 2 * e), sw),
        _pt(notch_face, snap_base_in_wall + hd * tip_h, sw),
        _pt(oc,         snap_base_in_wall + hd * tip_h, sw),
    ]
    solid = solid.cut(
        cq.Workplane(plane).workplane(offset=extrude_start - OVERCUT)
