...
- **Still applies to single-frame sub-components.** If the sub-component has no angled mounting, no physics-dependent profiles, and no multi-frame interfaces, the spatial resolution document will be short — but the agent must still produce it. Define the reference frame. Confirm no transforms are needed. List the interface positions in the local frame. Verify there are no hidden spatial dependencies. A short document that has been thought through is different from a skipped step.
