...
// posts whose hash changed since last boot. Same first-seen behavior as
// detectChangedSteps: a post seen for the first time records its hash and
// is NOT included in the returned list, so a fresh schema doesn't fire
// notifications for the entire backlog.
...
export async function notifyPostChanged({ postsDir, filename }) {
  if (!pool || !adminApp) return { sent: 0, removed: 0 };
  await ensureSchema();

  // Pull the post's title out of frontmatter for the notification heading.
    ...
    try {
      await messaging.send({
        ...
        webpush: {
          fcmOptions: { link },
          notification: {
            icon: "/dev/pwa-icons/icon-192.png",
            badge: "/dev/pwa-icons/icon-192.png",
          },
        },
      });
      sent++;
      ...
      if (
        ...
        console.error(`FCM send error for post ${filename}:`, e.message);
      }
    }
  }
